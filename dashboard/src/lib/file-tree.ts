...
export function useProjectTree(): ProjectTreeState {
  ...
  return { tree: tree, loading: loading, error: error };
}
...
export interface FileTreeState {
  ...
  loadingFiles: FileLoadingState;
}

export function useFileTreeState(_initialTree: TreeNode[]): FileTreeState {
  ...
  const openFile = useCallback(
    async (node: TreeNode) => {
      ...
      if (MODE === "standalone") {
        ...
        setOpenFiles((prev) => [
          ...prev,
          { path: node.path, name: node.name, content: mockContent, language: lang },
        ]);
        setActiveFilePath(node.path);
      ...
      try {
        ...
        setOpenFiles((prev) => [
          ...prev,
          { path: node.path, name: node.name, content: data.content, language: lang },
        ]);
        setActiveFilePath(node.path);
      } catch (err) {
        ...
        setOpenFiles((prev) => [
          ...prev,
          {
            path: node.path,
            name: node.name,
            content: `// Failed to load file: ${(err as Error).message}\n`,
            language: lang,
          },
        ]);
        setActiveFilePath(node.path);
...
function generateMockContent(name: string, lang: string): string {
  ...
  if (ext === "json")
    return JSON.stringify(
      { name: "example", version: "1.0.0", dependencies: { react: "^18.0.0" } },
      null,
      2,
    );
  if (ext === "md")
