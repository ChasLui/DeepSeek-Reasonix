...
export function MemoryPanel(): VNode | null {
  useLang();
  ...
  const fileRow = (scope: Scope, f: MemoryFile) => {
    ...
    return html`
      <div class=${`ssl-row ${sel ? "sel" : ""}`} onClick=${() => openFile(scope, f.name)}>
  ...
  const totalFiles =
    (tree.project.path ? 1 : 0) + tree.global.files.length + tree.projectMem.files.length;
      ...
      <div class="sessions-detail">
        ${open == null
          ? html`<div style="color:var(--fg-3);font-size:13px;text-align:center;padding:60px 20px">
              ${t("memory.pickHint")}
              <div style="margin-top:12px;font-size:11.5px">${t("memory.pickDesc")}</div>
            </div>`
          : html`
              <div class="sessions-detail-h">
                <span class="name"> ${open.scope}${open.name ? `/${open.name}` : ""} </span>
                <span class="ws"
                  >${t("memory.chars", { count: body.length.toLocaleString() })}</span
                >
                <span class="actions">
