...
function budgetKpi(o: OverviewData) {
  ...
  const valueColor =
    tone === "err" ? "color:var(--c-err)" : tone === "warn" ? "color:var(--c-warn)" : "";
  return html`
    <div class="kpi cock-w-1">
      <div class="label">${t("overview.budget")}</div>
      <div class="value" style=${valueColor}>${fmtUsd(state.spent)} / ${fmtUsd(state.cap)}</div>
      <div class=${`progress ${tone}`} style="margin-top:4px">
        <div class="progress-fill" style=${`width:${Math.min(100, state.pct)}%`}></div>
      </div>
    </div>
  `;
}
...
function toolCallsKpi(c: CockpitData) {
  if (!c.toolCalls24h)
    return kpi(t("overview.toolCalls24h"), "—", t("overview.noToolCalls"), "flat");
...
function currentSessionBlock(c: CockpitData) {
  ...
  return html`
    <div class="cock-list cock-w-2">
      <div class="ch"><span class="ttl">${t("overview.currentSession")}</span></div>
      <div
        class="card accent-brand"
        style="margin:0 0 8px;background:transparent;border:none;padding:0"
      >
        <div class="card-h">
          <span class="glyph">◆</span><span class="title">${s.id}</span
          ><span class="meta">${s.turns} turn${s.turns === 1 ? "" : "s"}</span>
        </div>
      </div>
      <div
        style="display:grid;grid-template-columns:repeat(3, 1fr);gap:8px;font-family:var(--font-mono);font-size:11px"
      >
        <div>
          <span style="color:var(--fg-3)">${t("overview.promptTok")}</span>
          <div style="color:var(--fg-0);font-size:13px;font-weight:600">
            ${fmtNum(s.lastPromptTokens)}
          </div>
        </div>
        <div>
          <span style="color:var(--fg-3)">${t("overview.completionTok")}</span>
          <div style="color:var(--fg-0);font-size:13px;font-weight:600">
            ${fmtNum(s.completionTokens)}
          </div>
        </div>
        <div>
          <span style="color:var(--fg-3)">${t("overview.cost")}</span>
          <div style="color:var(--fg-0);font-size:13px;font-weight:600">
            ${fmtCost(s.totalCostUsd, currency)}
          </div>
        </div>
      </div>
    </div>
...
function costTrendSpark(c: CockpitData) {
  if (!c.costTrend14d || c.costTrend14d.length === 0) {
    return html`
      <div class="chart cock-w-2">
        ...
        <div class="chart-v" style="color:var(--fg-4)">
          —<span class="unit">${t("overview.noUsageYet")}</span>
        </div>
      </div>
    `;
  }
  ...
  return html`
    <div class="chart cock-w-2">
      <div class="chart-h"><span class="title">${t("overview.costTrend")}</span></div>
      <div class="chart-v">
        ${fmtCost(avg, c.balance?.currency)}<span class="unit">${t("overview.dayAvg")}</span>
      </div>
  ...
  return html`
    <div class="cock-list cock-w-2">
      ...
      ${!c.recentPlans || c.recentPlans.length === 0
        ? html`<div style="color:var(--fg-3);font-size:12.5px;padding:8px 0">
            ${t("overview.noPlans")}
          </div>`
        : c.recentPlans.map(
            (p) => html`
              <div class=${`rail-step ${p.status === "done" ? "done" : "active"}`}>
                ...
                <span
                  >${p.title} · ${p.completedSteps}/${p.totalSteps}
                  step${p.totalSteps === 1 ? "" : "s"}</span
                >
                <span
                  style="margin-left:auto;color:var(--fg-4);font-family:var(--font-mono);font-size:10.5px"
                  >${fmtRelativeTime(p.whenMs)}</span
                >
              </div>
            `,
          )}
        ...
        : c.toolActivity.map(
            (r) => html`
              <div class=${`feed-row ${r.level}`}>
                ...
                <span class="name"
                  >${r.name}${r.args ? html` <span class="args">${r.args}</span>` : null}</span
                >
...
export function OverviewPanel(): VNode | null {
  useLang();
  ...
  if (error)
    return html`<div class="card accent-err">
      ${t("overview.failed", { error: error.message })}
    </div>`;
  if (!data) return null;
  const o = data;
        ...
        ? html`<div class="card accent-warn">
            <div class="card-h">
              <span class="title" style="color:var(--c-warn)"
                >${t("overview.standaloneTitle")}</span
              >
            </div>
            <div class="card-b">${t("overview.standaloneDesc")}</div>
          </div>`
        : null}

      <h3
        style="margin:0;font-family:var(--font-mono);font-size:11px;color:var(--fg-3);text-transform:uppercase;letter-spacing:.1em"
      >
        ${t("overview.cockpit")}
      </h3>
      <div class="cockpit">
        ${balanceKpi(c)} ${tokens7dKpi(c)} ${cacheHitKpi(c)} ${toolCallsKpi(c)} ${budgetKpi(o)}
        ${currentSessionBlock(c)} ${costTrendSpark(c)} ${recentPlansRail(c)} ${toolActivityFeed(c)}
        ${kpi(
          t("overview.toolsLoaded"),
          fmtNum(o.toolCount),
          o.toolCount ? t("overview.active") : "—",
          "flat",
        )}
        ${kpi(
          t("overview.mcpServers"),
          fmtNum(o.mcpServerCount),
          o.mcpServerCount ? t("overview.allUp") : "—",
          o.mcpServerCount ? "up" : "flat",
        )}
        ${kpi(
          t("overview.editMode"),
          o.editMode ?? "—",
          o.editMode === "yolo" ? t("overview.yoloWarning") : null,
