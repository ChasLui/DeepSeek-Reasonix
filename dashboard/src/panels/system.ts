...
export function SystemPanel(): VNode | null {
  useLang();
  ...
  if (error)
    return html`<div class="card accent-err">
      ${t("common.loadingFailed", { name: "health", error: error.message })}
    </div>`;
  if (!data) return null;
  const h = data;
    ...
    <div style="display:flex;flex-direction:column;gap:14px">
      ...
      <div class="health-grid">
        ...
        <div class="health-item">
          <div class="lbl">
            ${t("system.sessions")} <span class="pill ok">${t("system.ok")}</span>
          </div>
        ...
        <div class="health-item">
          <div class="lbl">
            ${t("system.memory")} <span class="pill ok">${t("system.ok")}</span>
          </div>
        ...
        <div class="health-item">
          ...
          <div class="meta">
            ${h.semantic.exists
              ? `${fmtNum(h.semantic.fileCount)} ${t("system.files")}`
              : t("system.runIndex")}
          </div>
        </div>
        ...
        <div class="health-item">
          <div class="lbl">
            ${t("system.usageLog")} <span class="pill ok">${t("system.ok")}</span>
          </div>
        ...
        <div class="health-item">
          ...
          <div class="v">
            ${h.jobs === null ? "—" : t("system.running", { count: fmtNum(h.jobs) })}
          </div>
          <div class="meta">
            ${h.jobs === null ? t("system.attachHint") : t("system.shellSpawn")}
          </div>
        </div>
      </div>
      ...
      <div class="card" style="padding:0">
        ...
        <table class="tbl">
          <tbody style="font-size:11.5px">
            <tr>
              <td class="dim" style="padding:5px 14px">${t("system.home")}</td>
              <td class="path">${h.reasonixHome}</td>
            </tr>
            <tr>
              <td class="dim" style="padding:5px 14px">${t("system.sessionsPath")}</td>
              <td class="path">${h.sessions.path}</td>
            </tr>
            <tr>
              <td class="dim" style="padding:5px 14px">${t("system.memoryPath")}</td>
              <td class="path">${h.memory.path}</td>
            </tr>
            <tr>
              <td class="dim" style="padding:5px 14px">${t("system.semanticPath")}</td>
              <td class="path">${h.semantic.path}</td>
            </tr>
            <tr>
              <td class="dim" style="padding:5px 14px">${t("system.usagePath")}</td>
              <td class="path">${h.usageLog.path}</td>
            </tr>
          </tbody>
        </table>
      </div>
