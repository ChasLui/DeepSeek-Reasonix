...
function buildPlatformNotes(version) {
  return {
    mac: {
      ...
      steps: [
        {
          cmd: "xattr -dr com.apple.quarantine /Applications/Reasonix.app",
          note: { zh: "终端一行解除隔离属性", en: "One-liner to clear the quarantine attribute" },
        },
        {
          cmd: "right-click → Open → confirm",
          note: {
            zh: "在 Finder 中右键打开，确认一次后续不再询问",
            en: "Right-click → Open → confirm once; macOS remembers afterwards",
          },
        },
      ],
    },
    win: {
      ...
      steps: [
        {
          cmd: "More info → Run anyway",
          note: {
            zh: '点 "更多信息" 然后 "仍要运行" 即可',
            en: 'Click "More info", then "Run anyway"',
          },
        },
        {
          cmd: "Get-AuthenticodeSignature .\\Reasonix_setup.exe",
          note: {
            zh: "可在 PowerShell 中校验文件 hash",
            en: "Verify the file hash in PowerShell if you want",
          },
        },
      ],
    },
    linux: {
      ...
      steps: [
        {
          cmd: `chmod +x Reasonix_${version}_amd64.AppImage`,
          note: { zh: "赋予可执行权限", en: "Mark it executable" },
        },
        {
          cmd: "sudo apt install libfuse2 # debian/ubuntu",
          note: { zh: "AppImage 运行时依赖", en: "AppImage runtime dependency" },
        },
      ],
    },
  };
...
function DownloadHero() {
  ...
  const heroBadge =
    rxStatus === "ok" && rxVersion
      ...
      : t(
          { zh: "Reasonix Desktop · 正在获取版本…", en: "Reasonix Desktop · fetching version…" },
          lang,
        );
  return (
            ...
            {t(
              {
                zh: (
                  <>
                    原生 <b>Tauri</b> 客户端 · 自带 Node runtime · 共享 <b>~/.reasonix</b>{" "}
                    配置与会话。多 tab 并行，右侧栏列出当前会话读过和改过的文件，底部 cost / cache /
                    token 实时表盘。
                  </>
                ),
                en: (
                  <>
                    Native <b>Tauri</b> client · bundled Node runtime · shares <b>~/.reasonix</b>{" "}
                    config + history with the CLI. Multi-tab sessions, side panel listing files read
                    / edited this session, live cost / cache / token meters along the bottom.
                  </>
                ),
              },
              lang,
  ...
  return (
    <section className="section" id="cli-alt">
      ...
      <div className="copy-block" style={{ maxWidth: 640 }}>
        <span className="cmd">
          <span className="tok-cmt">$ </span>cd /path/to/my-project &amp;&amp; npx reasonix code
        </span>
      </div>
    ...
    <>
      ...
      <section className="section" id="mirror">
        <div className="sec-meta">
          ...
          <span>
            · {t({ zh: "Smart Mirror · 自动测速", en: "Smart mirror · auto probe" }, lang)}
          </span>
          <span className="rule"></span>
        </div>
        <div className="section-head">
