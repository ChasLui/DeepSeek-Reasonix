...
const buildTermScript = (version) => [
  ...
  {
    t: "out",
    text: `⏺ reasonix ${version} · model: deepseek-v4-flash · workspace: ~/app`,
    cls: "term-info",
    delay: 280,
  },
  {
    t: "out",
    text: "⏺ cache: 94.2% hit · session: 18m23s · cost: $0.043",
    cls: "term-dim",
    delay: 220,
  },
  { t: "blank" },
  ...
  {
    t: "out",
    text: "▸ tool<search_files>  → src/users.ts, src/users.test.ts",
    cls: "term-dim",
    delay: 260,
  },
  {
    t: "out",
    text: "▸ tool<read_file>     → src/users.ts (412 chars)",
    cls: "term-dim",
    delay: 220,
  },
  ...
  {
    t: "out",
    text: "  return users.find(u => u.email.toLowerCase() === needle);",
    cls: "term-ok",
    delay: 80,
  },
  ...
  {
    t: "out",
    text: "▸ 1 pending edit · /apply to write · /discard to drop",
    cls: "term-info",
    delay: 240,
  },
  { t: "blank" },
    ...
    <div className="terminal" role="presentation" aria-hidden="true">
      ...
      <div className="term-body">
        ...
        {typing !== "" ? (
          ...
          lines.length > 0 &&
          stepRef.current < TERM_SCRIPT.length && (
            <div className="term-line">
              <span className="term-prompt-user">›</span>
              <span className="cursor"></span>
            </div>
            ...
            {t(
              {
                zh: (
                  <>
                    Reasonix 直接对接 <b>api.deepseek.com</b>，围绕 DeepSeek 的字节稳定 prefix-cache
                    设计了 append-only 的运行循环 —— 长会话能把缓存命中保持在 90%+，输入 token
                    成本降到 1/5。终端优先，留它一直跑着。
                  </>
                ),
                en: (
                  <>
                    Reasonix talks straight to <b>api.deepseek.com</b>. The loop is append-only,
                    engineered around DeepSeek's byte-stable prefix cache — long sessions hold 90%+
                    cache hit and input-token cost collapses to ~1/5. Terminal-first, leave it
                    running.
                  </>
                ),
              },
