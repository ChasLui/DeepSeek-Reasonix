...
const CONFIG_TABS = [
  {
    ...
    bullets: [
      ...
      {
        zh: "所有 MCP 工具沙箱权限与原生工具一致",
        en: "MCP tools share the same sandbox as built-ins",
      },
      {
        zh: "/mcp 子命令查看已挂载服务器 · 健康状态 · 工具清单",
        en: "`/mcp` lists mounted servers · health · tools",
      },
      {
        zh: "失败重连 · 自动 reconnect with backoff",
        en: "Auto-reconnect on failure with exponential backoff",
      },
    ],
    files: [
      {
  ...
  },
  {
    ...
    bullets: [
      {
        zh: "项目级: <project>/.reasonix/skills/<name>.md",
        en: "Project: <project>/.reasonix/skills/<name>.md",
      },
      ...
      {
        zh: "runAs: subagent 让 skill 跑在隔离的子循环里",
        en: "`runAs: subagent` runs the body in an isolated sub-loop",
      },
    ],
    files: [
      {
...
› please review the current branch`,
  ...
  },
  {
    ...
    bullets: [
      {
        zh: "<project>/reasonix.md · 项目约定 · git-tracked",
        en: "<project>/reasonix.md · project conventions · git-tracked",
      },
      {
        zh: "~/.reasonix/memory.md · 用户偏好 · 私有",
        en: "~/.reasonix/memory.md · user preferences · private",
      },
      { zh: "/memory edit 在 TUI 内直接编辑", en: "/memory edit opens it inside the TUI" },
      {
        zh: "注入位置位于 cache-stable 前缀 · 不影响命中",
        en: "Injected inside the cache-stable prefix · cache hit unaffected",
      },
    ],
    files: [
      {
...
- Small commits · one thing at a time`,
  ...
  },
  {
    ...
    bullets: [
      {
        zh: "/pro · /preset · /effort   — 模型与推理深度切换",
        en: "/pro · /preset · /effort   — switch model + reasoning depth",
      },
      {
        zh: "/plan · /apply · /discard  — 编辑审批门",
        en: "/plan · /apply · /discard  — edit approval gate",
      },
      {
        zh: "/mcp · /skill · /memory    — 外部能力与剧本管理",
        en: "/mcp · /skill · /memory    — capabilities + playbooks",
      },
      {
        zh: "/status · /stats · /replay — 会话状态与回放",
        en: "/status · /stats · /replay — session state + replay",
      },
    ],
    files: [
      {
...
function CodePanel({ file }) {
  ...
  return (
    <div className="code-panel">
      ...
      <pre
        className="code-body"
        dangerouslySetInnerHTML={{ __html: syntaxHighlight(file.code, file.lang) }}
      />
    </div>
  );
}
      ...
      <div className="config-grid">
        <div className="config-side">
          {CONFIG_TABS.map((it) => (
            <div
              key={it.id}
              className={"config-tab " + (it.id === tab ? "on" : "")}
              ...
              {t(
                {
                  ...
                  en: (
                    <>
                      Every path and command shown lives in{" "}
                      <a
                        href="https://github.com/esengine/DeepSeek-Reasonix"
                        target="_blank"
                        rel="noreferrer"
